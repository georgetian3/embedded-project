...
int ap_open(AudioPlayer* ap, char* filename) {
    ...
    /* printf("%d %d %d %d\n", sizeof(WaveHeader), ap->header.data_chunk.Size, sizeof(WaveHeader) + ap->header.data_chunk.Size,  file_size);

    if (sizeof(WaveHeader) + ap->header.data_chunk.Size != file_size) {
    ...
    if (ap->header.riff_chunk.ID     != WAVEHEADER_RIFF_CHUNK_ID    ||
        ap->header.riff_chunk.Format != WAVEHEADER_RIFF_WAVE_FORMAT ||
...
int ap_play(AudioPlayer* ap, double timestamp, double speed, bool blocking) {
    ...
    if (error = snd_pcm_set_params(
            ...
            ap->header.format_chunk.SampleRate, 1, 500000
        )) {
